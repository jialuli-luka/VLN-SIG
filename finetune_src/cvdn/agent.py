    ...
    def rollout_ig(self, train_ml=None, train_rl=True, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.args.weighted_token:
                visual_inputs['position_id'] = torch.from_numpy(np.ones((batch_size, 196), dtype=np.float32)/196).to(txt_embeds[0].get_device())
            ...
            if train_ml is not None:
                if self.args.weighted_token:
                    ...
                    logit_dvae = torch.matmul(ig_probs.unsqueeze(1), cand_probs).squeeze(1)
            ...
            if train_ml is not None:
                ...
                target = self._teacher_action(obs, ended)
                ml_loss += self.criterion(logit, target)
                ml_loss += self.criterion(logit_dvae, target)
